...
impl AbstractState {
    ...
    &&& forall|i: int| {
        0 <= i && i < MAX_NR_PAGES() ==> {
                #[trigger] self.pages.dom().contains(i) &&
    ...
    {
        exists |node: PageTableNodeModel|
            #[trigger]
            self.page_table.tree@.on_tree(node@)
    }
    ...
    pub open spec fn pt_nodes_usage_invariant(self) -> bool
    {
        forall |paddr: Paddr|
            #[trigger]
            self.is_node(paddr) ==>
            self.get_page(paddr).usage == PageUsage::PageTable
    }

    #[verifier::returns(proof)]
    pub proof fn borrow_pt_model(#[verifier::proof] &self) -> (res: Tracked<&PageTableModel>)
    {
...
/*    pub open spec fn update_page_table_idx(&self, idx: int, pte:PageTableEntry) -> Self
    ...
    pub open spec fn update_page_model_spec(self, paddr: Paddr, model: PageModel) -> AbstractState
    {
        ...
        AbstractState {
            pages: self.pages.insert(index,model),
            ..self
        }
    } 
...
pub fn panic(Tracked(s): Tracked<AbstractState>, msg: &str) -> 
    (res: Tracked<AbstractState>)
    ensures
        res@ == s.panic_spec(msg),
{
    Tracked(AbstractState {
        errors: s.errors.push(msg),
        ..s
    })
}
