...
pub const PAGE_TABLE_NODE_OWNER: PageOwner = PageOwner::PageTable(PageTablePageOwner::PageTableNode);
pub const RAW_PAGE_TABLE_NODE_OWNER: PageOwner = PageOwner::PageTable(PageTablePageOwner::RawPageTableNode);
    ...
    {   
        ...
        &&& match self.specific_perm {
            ...
            SpecificPagePerm::NoPerm => {
                is_variant(self.get_inner_perm().value(),"_frame")
            },
        }
    }
}
...
pub open spec fn get_pagetable_inner_perm(self) -> cell::PointsTo<PageTablePageMetaInner>
    recommends self.specific_perm is PT,
{
...
pub proof fn tracked_borrow_inner_perm (tracked & self) -> (tracked res: &cell::PointsTo<MetaSlotInner>)
    requires
        self.inner_perm.is_some(),
    ensures
...
pub proof fn tracked_borrow_pagetable_inner_perm(tracked & self) -> (tracked res: &cell::PointsTo<PageTablePageMetaInner>)
    requires
...
pub proof fn tracked_borrow_pagetable_model(tracked & self) -> (tracked res: &PageTablePageMetaModel)
    requires
        self is PT,
    ensures
...
pub proof fn tracked_unwrap_pagetable_model(tracked self) -> (tracked res: PageTablePageMetaModel)
    requires
        self is PT,
    ensures
        res == self->PT_0,
