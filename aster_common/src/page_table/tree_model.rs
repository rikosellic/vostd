...
impl PageTableNodeModel {
    pub open spec fn valid_ptrs(&self) -> bool {
        forall |i:usize|
            #[trigger]
            between(0, CONST_NR_ENTRIES, i) ==>
            forall |child: Node<PageTableNodeValue, CONST_NR_ENTRIES, CONST_NR_LEVELS>|
                ...
                self.inner.value.perms.unwrap().opt_value()[i as int].value().paddr() == child.value.paddr
    }

    pub open spec fn from_node(node: ghost_tree::Node<PageTableNodeValue, CONST_NR_ENTRIES, CONST_NR_LEVELS>)
        -> PageTableNodeModel
    {
        PageTableNodeModel {
            inner: node
...
impl PageTableTreeModel {
    ...
    &&& self.inner.inv()
    &&& forall |node: PageTableNodeModel|
            #[trigger]
            self.inner.on_tree(node@) ==>
            node@.inv()
    }
}
}