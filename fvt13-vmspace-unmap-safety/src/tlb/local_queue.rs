...
use aster_common::x86_64::mm::{tlb_flush_addr, tlb_flush_addr_range, tlb_flush_all_excluding_global};
...
impl TlbFlushOp {
    ...
    pub open spec fn vaddr_set_spec(self) -> Seq<int>
        recommends
            !self.is_flush_all(),
    {
        if let Self::Address(vaddr) = self {
            Seq::empty().push(vaddr as int)
        } else if let Self::Range(r) = self {
            Seq::new((r.end - r.start) as nat, |i: int| { r.start + i })
        } else {
            Seq::empty()
        }
    }
    ...
    pub proof fn axiom_vaddr_set_spec(self)
        requires
            !self.is_flush_all(),
        ensures
            if let Self::Address(vaddr) = self {
...
impl OpsStackModel {
    ...
    pub open spec fn invariants(&self) -> bool {
        ...
        &&& 0 <= self.size_spec() < FLUSH_ALL_OPS_THRESHOLD
        &&& self.need_flush_all_spec() ==> self.size_spec() == 0
        &&& !self.ops.contains(TlbFlushOp::All)
    }

    pub open spec fn vaddr_set_spec(&self) -> Seq<int> {
        self.ops.map_values(|op: TlbFlushOp| { op.vaddr_set_spec() }).flatten()
    }

    #[verifier::external_body]
    // Here needs a lemma of 'vstd::Seq::flatten'
    // We describe what the set should be.
    pub proof fn axiom_vaddr_set_subset(self, vaddr_set: Seq<int>)
        requires
            vaddr_set =~= self.vaddr_set_spec(),
        ensures
            forall|i: int|
                #![auto]
                0 <= i < self.ops.len() ==> self.ops[i].vaddr_set_spec().to_set().subset_of(
                    vaddr_set.to_set(),
                ),
    {
    }
    ...
    pub open spec fn push(self, op: TlbFlushOp) -> Self {
        if op.is_flush_all() {
            ...
            let need_flush_all = new_size == FLUSH_ALL_OPS_THRESHOLD;
    ...
    pub proof fn lemma_tlb_flush(self, tlb: TlbModel) -> (res: (Self, TlbModel))
        ...
        ensures
            ...
            res.0.invariants(),
            forall|i: int| #![auto] 0 <= i < self.size_spec() ==> { res.1.op_issued(self.ops[i]) },
    {
        let new_tlb = if self.need_flush_all_spec() {
            tlb.clear()
        } else {
            let va_set = self.ops.map_values(|op: TlbFlushOp| { op.vaddr_set_spec() }).flatten();
            let new_tlb = tlb.flush_va_set(va_set);

            assert(forall|i: int|
                #![auto]
                0 <= i < self.size_spec() ==> self.ops[i].vaddr_set_spec().to_set().subset_of(
                    va_set.to_set(),
                )) by {
                self.axiom_vaddr_set_subset(va_set);
            };
...
pub tracked struct KeptPageModel {
    pub ghost page_perm: PointsTo<DynPage>,
    pub ghost owner_cpus: Set<int>,
    pub tracked op: TlbFlushOp,
}

impl KeptPageModel {
    pub open spec fn paddr_spec(&self) -> Paddr {
        self.page_perm.value().paddr_spec()
    }
    ...
    pub open spec fn inv_owner(&self, page: &PageModel) -> bool {
        &&& page.relate_paddr(self.paddr_spec())
        &&& match page.state {
    ...
    pub open spec fn drop_spec(self, page: PageModel, cpu_id: int) -> (Self, PageModel) {
        (
            Self {
            ...
            PageModel { ref_count: page.ref_count - 1, ..page },
        )
    }

    pub proof fn lemma_drop_spec(self, page: PageModel, cpu_id: int) -> (res: (Self, PageModel))
        requires
            self.inv_owner(&page),
...
pub tracked struct KeptPageVecModel {
    pub tracked vec: Seq<KeptPageModel>,
}
...
impl KeptPageVecModel {
    ...
    pub open spec fn clear(self, pages: Map<int, PageModel>, cpu_id: int) -> (
        Self,
        Map<int, PageModel>,
    ) {
        let new_pages = pages.map_entries(
            |paddr: int, page: PageModel|
                {
    ...
    pub proof fn lemma_clear(self, pages: Map<int, PageModel>, cpu_id: int) -> (res: (
        Self,
        Map<int, PageModel>,
    ))
        requires
            ...
            forall|i: int| #![auto] 0 <= i < MAX_NR_PAGES() <==> pages.dom().contains(i),
            forall|i: int|
                {
                    0 <= i < MAX_NR_PAGES() ==> {
                        #[trigger] pages.dom().contains(i) && pages[i].index == i
                            && pages[i].invariants()
                    }
                },
        ensures
            ...
            res.0.invariants(),
            forall|i: int| #![auto] 0 <= i < MAX_NR_PAGES() <==> res.1.dom().contains(i),
            forall|i: int|
                {
                    0 <= i < MAX_NR_PAGES() ==> {
                        #[trigger] res.1.dom().contains(i) && res.1[i].index == i
                            && res.1[i].invariants()
                    }
                },
    {
        ...
        //     }
        // });
        assert(forall|pa: int|
            #![auto]
            self.paddr_set_spec().contains(pa) ==> {
...
pub tracked struct AbstractModel {
    pub ghost tlb_queues: Seq<TlbQueueModel>,
    pub ghost tlbs: Seq<TlbModel>,
    pub ghost pages: Map<int, PageModel>,
}

impl AbstractModel {
    pub open spec fn inv_tlb(&self) -> bool {
        &&& self.tlb_queues.len() == cpu_num()
        &&& self.tlbs.len() == cpu_num()
        &&& forall|i: int|
            0 <= i < self.tlb_queues.len() ==> #[trigger self.tlb_queues[i]]
            self.tlb_queues[i].invariants()
    }

    pub open spec fn inv_page(&self) -> bool {
        &&& forall|i: int| #![auto] 0 <= i < MAX_NR_PAGES() <==> self.pages.dom().contains(i)
        &&& forall|i: int|
            {
                0 <= i < MAX_NR_PAGES() ==> {
        ...
        &&& forall|i: int|
            #![auto]
            0 <= i < self.tlb_queues.len() ==> {
                self.tlb_queues[i].page_keeper.inv_owner(&self.pages)
            }
    }
    ...
    pub open spec fn invariants(&self) -> bool {
        ...
        &&& forall|i: int|
            ...
            0 <= i < self.tlb_queues.len() ==> {
                &&& self.tlb_queues[i].invariants()
                &&& self.tlb_queues[i].page_keeper.cpu_is_owner(i)
                &&& self.tlb_queues[i].page_keeper.inv_owner(&self.pages)
            }
    }
}

pub tracked struct TlbQueueModel {
    pub tracked flush_ops: OpsStackModel,
    pub tracked page_keeper: KeptPageVecModel,
}

impl TlbQueueModel {
    pub open spec fn inv_kept_page_all_have_owner(&self) -> bool {
        let flush_ops = self.flush_ops;
        let page_keeper = self.page_keeper;

        ||| self.flush_ops.need_flush_all_spec()
        ||| forall|i: int|
            #![auto]
            0 <= i < self.page_keeper.vec.len() ==> {
                self.flush_ops.ops.contains(self.page_keeper.vec[i].op)
            }
    }

    pub open spec fn invariants(&self) -> bool {
        &&& self.flush_ops.invariants()
        &&& self.page_keeper.invariants()
        &&& self.inv_kept_page_all_have_owner()
    }
}
