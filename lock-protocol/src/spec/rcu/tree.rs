...
fn protocol_lock_start_inductive(pre: Self, post: Self, cpu: CpuId, nid: NodeId) {
    ...
    assert(post.wf_cursors()) by {
        assert forall |cpu_id: CpuId| #[trigger] post.cursors.dom().contains(cpu_id) implies {
            ...
            if cpu_id == cpu {
                ...
                if post.cursors.dom().contains(cpu_id) {
                    assert(pre.cursors.dom().contains(cpu_id));
    ...
    assert(post.inv_non_overlapping()) by {
        assert forall |cpu1: CpuId, cpu2: CpuId|
            ...
            if cpu1 == cpu {
                ...
                if post.cursors.dom().contains(cpu1) && post.cursors.dom().contains(cpu2) {
...
fn protocol_lock_inductive(pre: Self, post: Self, cpu: CpuId, nid: NodeId) {
    ...
    assert(post.wf_nodes()) by {
        assert forall |node_id: NodeId| #[trigger] post.nodes.dom().contains(node_id) implies
            ...
            if node_id == nid {
                ...
                if post.nodes.dom().contains(node_id) {
                    assert(pre.nodes.dom().contains(node_id));
    ...
    assert(post.inv_pt_node_pte_array_relationship()) by {
        assert forall |node_id: NodeId| post.nodes.dom().contains(node_id) <==> post.pte_arrays.dom().contains(node_id) by {
            if node_id == nid {
                ...
                assert(post.nodes.dom().contains(node_id));
                ...
                if nid != NodeHelper::root_id() {
                    ...
                    assert(pre.pte_arrays.dom().contains(nid));
                    assert(post.pte_arrays.dom().contains(nid));
                } else {
                ...
                // Other nodes unchanged
                assert(post.nodes.dom().contains(node_id) == pre.nodes.dom().contains(node_id));
                assert(post.pte_arrays.dom().contains(node_id) == pre.pte_arrays.dom().contains(node_id));
            }
        }
    };
    ...
    assert(post.inv_pt_node_pte_relationship()) by {
        ...
        assert forall |node_id: NodeId|
            ...
            if node_id == nid && nid != NodeHelper::root_id() {
                ...
                assert(post.pte_arrays.dom().contains(pa) && post.pte_arrays[pa].is_alive(offset));
            } else if node_id != nid {
                // Other nodes unchanged
                assert(post.nodes.dom().contains(node_id) == pre.nodes.dom().contains(node_id));
    ...
    assert(post.inv_pt_node_pte_array_relationship()) by {
        assert forall |node_id: NodeId| post.nodes.dom().contains(node_id) <==> post.pte_arrays.dom().contains(node_id) by {
            if node_id == nid {
                ...
                assert(post.nodes.dom().contains(node_id)); // nid is Free, still in nodes
                assert(post.pte_arrays.dom().contains(node_id)); // unchanged
            } else {
    ...
    assert(post.inv_pt_node_pte_array_relationship()) by {
        assert forall |node_id: NodeId| post.nodes.dom().contains(node_id) <==> post.pte_arrays.dom().contains(node_id) by {
            if node_id == nid {
                ...
                // pte_arrays is unchanged, so nid remains there too
                assert(post.nodes.dom().contains(node_id)); // nid is LockedOutside, still in nodes
                assert(post.pte_arrays.dom().contains(node_id)); // unchanged
            } else {
                // Other nodes unchanged in both maps
                assert(post.nodes.dom().contains(node_id) == pre.nodes.dom().contains(node_id));
