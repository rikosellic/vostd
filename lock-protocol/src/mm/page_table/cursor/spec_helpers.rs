...
pub open spec fn instance_match(mpt: &exec::MockPageTable, tokens: exec::Tokens) -> bool {
    &&& forall|i|
...
pub open spec fn mpt_and_tokens_wf(mpt: &exec::MockPageTable, tokens: exec::Tokens) -> bool {
    &&& instance_match(mpt, tokens)
    &&& forall|i: usize|
        0 <= i < exec::MAX_FRAME_NUM && mpt.mem@[i].1@.is_uninit()
    ...
    &&& forall|i: usize|
        0 <= i < exec::MAX_FRAME_NUM && #[trigger] mpt.mem@[i].1@.is_init() ==> forall|j: usize|
            0 <= j < NR_ENTRIES ==> #[trigger] mpt.mem@[i].1@.value().ptes[j as int].frame_pa == 0
                ==> #[trigger] tokens.unused_pte_addrs.contains_key(
                mpt.mem@[i].1@.value().ptes[j as int].frame_pa as int,
            )
}
...
pub open spec fn mpt_not_contains_not_allocated_frames(
    ...
    &&& forall|i: usize|
        cur_alloc_index <= i < exec::MAX_FRAME_NUM ==> !mpt.frames@.value().contains_key(
            #[trigger] exec::frame_index_to_addr(i) as int,
        ) && forall|j: usize|
            0 <= j < NR_ENTRIES ==> !#[trigger] mpt.ptes@.value().contains_key(
                exec::frame_addr_to_index(i) + j * exec::SIZEOF_PAGETABLEENTRY as int,
            )
    &&& forall|i: usize|
        cur_alloc_index <= i < exec::MAX_FRAME_NUM ==> forall|j: int| #[trigger]
            mpt.ptes@.value().contains_key(j) ==> mpt.ptes@.value()[j].frame_pa
                != #[trigger] exec::frame_index_to_addr(i) as int
}
...
pub open spec fn frame_keys_do_not_change(
    ...
    &&& forall|i|
        old_mpt.frames@.value().contains_key(i) ==> #[trigger] mpt.frames@.value().contains_key(i)
    &&& forall|i|
        mpt.frames@.value().contains_key(i) ==> #[trigger] old_mpt.frames@.value().contains_key(i)
    &&& forall|i|
        !old_mpt.frames@.value().contains_key(i) ==> !#[trigger] mpt.frames@.value().contains_key(i)
    &&& forall|i|
        !old_mpt.frames@.value().contains_key(i) ==> !#[trigger] mpt.frames@.value().contains_key(i)
...
pub open spec fn pte_keys_do_not_change(
    ...
    &&& forall|i|
        old_mpt.ptes@.value().contains_key(i) ==> #[trigger] mpt.ptes@.value().contains_key(i)
    &&& forall|i|
        mpt.ptes@.value().contains_key(i) ==> #[trigger] old_mpt.ptes@.value().contains_key(i)
    &&& forall|i|
        !old_mpt.ptes@.value().contains_key(i) ==> !#[trigger] mpt.ptes@.value().contains_key(i)
    &&& forall|i|
        !old_mpt.ptes@.value().contains_key(i) ==> !#[trigger] mpt.ptes@.value().contains_key(i)
