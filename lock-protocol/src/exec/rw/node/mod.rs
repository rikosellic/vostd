...
impl PageTableNode {
    ...
    pub uninterp spec fn from_raw_spec(
        paddr: Paddr,
    ) -> Self;
...
impl PageTableNode {
    ...
    ) -> (res: (PageTableWriteLock, Tracked<LockProtocolModel>))
        requires
            self.wf(mem),
            m@.inv(),
    ...
    pub fn lock_read(
        ...
        mem: &MemContent,
        m: Tracked<LockProtocolModel>,
    ) -> (res: (PageTableReadLock, Tracked<LockProtocolModel>))
        requires
            self.wf(mem),
            m@.inv(),
            m@.inst_id() == self.inst@.id(),
...
impl PageTableReadLock {
    ...
    pub fn unlock(
        ...
        mem: &MemContent,
        m: Tracked<LockProtocolModel>,
    ) -> (res: (PageTableNode, Tracked<LockProtocolModel>))
        requires
            old(self).wf(mem),
            m@.inv(),
...
impl PageTableWriteLock {
    ...
    pub proof fn tracked_pt_inst(
        tracked &self,
        mem: &MemContent
    ) -> (tracked res: SpecInstance)
        requires
            self.wf(mem),
        ensures
