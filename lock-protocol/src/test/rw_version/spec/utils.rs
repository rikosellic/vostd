    ...
    decreases max_dep,
{
    ...
    if max_dep == 0 { Self::size_at_dep(cur_dep) }
    else if max_dep > 0 { Self::size_at_dep(cur_dep) + Self::tree_size_spec(max_dep - 1) }
    else { arbitrary() }
}
...
pub open spec fn nid_to_trace(nid: NodeId) -> Seq<nat>
    recommends
        Self::valid_nid(nid),
{
    if nid == Self::root_id() { Seq::empty() }
    else { Self::nid_to_trace_rec(nid, 3, Self::root_id()) }
}

pub open spec fn valid_trace(trace: Seq<nat>) -> bool {
    &&& 0 <= trace.len() < 4
    &&& forall |i| #![trigger trace[i]] 
        0 <= i < trace.len() ==> 0 <= trace[i] < 512
}
...
pub open spec fn trace_to_nid(trace: Seq<nat>, cur_rt: NodeId, cur_level: int) -> NodeId 
    recommends
        ...
        0 <= cur_level <= 3,
{
    trace.fold_left_alt((cur_rt, cur_level as int), |param: (NodeId, int), offset: nat| {
        let nid = param.0;
...
pub proof fn lemma_trace_to_nid_inductive(trace: Seq<nat>, cur_rt: NodeId, cur_level: int)
    ...
    ensures
        if trace.len() > 0 {
            Self::trace_to_nid(trace, cur_rt, cur_level) ==
            Self::trace_to_nid(
    ...
    decreases max_dep,
{
    ...
    if cur_dep == 0 { 0 }
        ...
        Self::tree_size_weighted_spec(level - 1) + Self::trace_to_nid_upperbound_spec((max_dep - 1) as nat)
    }
}
...
pub proof fn lemma_trace_to_nid_upperbound_spec(max_dep: nat)
    requires
        0 <= max_dep < 4,
    ensures
        Self::trace_to_nid_upperbound_spec(max_dep) == 
        Self::tree_size_spec(3) - Self::tree_size_spec(3 - max_dep),
    decreases max_dep,
{
...
pub proof fn lemma_trace_to_nid_inner(trace: Seq<nat>)
    requires
        Self::valid_trace(trace),
    ensures
        trace.fold_left_alt((0, 3), |param: (NodeId, int), offset: nat| {
            let nid = param.0;
            let cur_level = param.1;
            let sz = Self::tree_size_spec(cur_level - 1);
            (nid + offset * sz + 1, cur_level - 1)
    ...
    decreases trace.len(),
{
    ...
    if trace.len() != 0 {
        ...
        lemma_mul_inequality(trace.last() as int, 511, Self::tree_size_spec(3 - trace.len()) as int);
        ...
        Self::trace_to_nid_increment_spec(trace, offset),
{
    ...
    assert(func(trace.fold_left_alt((0, 3), func), offset) == 
        trace.push(offset).fold_left_alt((0, 3), func)
    ) by {
        trace.lemma_fold_left_alt((0, 3), func);
...
pub proof fn lemma_trace_to_nid_split(trace1: Seq<nat>, trace2: Seq<nat>, cur_rt: NodeId, cur_level: int) -> (nid: NodeId)
    requires
    ...
    decreases cur_level,
{
    if cur_level == 0 { Seq::empty() }
        ...
        assert(Self::tree_size_spec(cur_level - 1) * 512 + 1 == 
            Self::tree_size_spec(cur_level as int)
        ...
        assert(0 <= offset < 512) by {
            ...
            assert(offset < 512) by { 
                lemma_div_by_multiple_is_strongly_ordered(
                    offset * sz as int, 512 * sz as int, 512, sz as int
                );
...
pub proof fn lemma_trace_to_nid_rec(trace: Seq<nat>, cur_rt: NodeId, cur_level: int) -> (nid: NodeId)
    requires
        Self::valid_trace(trace),
        Self::valid_nid(cur_rt),
    ...
    decreases trace.len(),
{
    if trace.len() == 0 {
        ...
        assert({
            ...
            assert(Self::tree_size_spec(cur_level) - 1 - trace[0] * Self::tree_size_spec(cur_level - 1) ==
                (512 - trace[0]) * Self::tree_size_spec(cur_level - 1)
            ) by { 
            ...
            assert(new_rt + (512 - trace[0]) * Self::tree_size_spec(cur_level - 1) <= Self::total_size());
            assert(Self::tree_size_spec(cur_level - 1) <= (512 - trace[0]) * Self::tree_size_spec(cur_level - 1)) by {
        ...
        nid + Self::tree_size_spec(level - 1) <= Self::total_size(),
{
    ...
    assert(Self::trace_to_nid_upperbound_spec(trace.len()) == 
        Self::tree_size_spec(3) - Self::tree_size_spec(3 - trace.len())
    ) by { Self::lemma_trace_to_nid_upperbound_spec(trace.len()) };
}
...
{
    let trace = Self::nid_to_trace(nid);
    assert(Self::valid_trace(trace)) by { Self::lemma_nid_to_trace(nid); };
    ...
    let _nid = Self::lemma_trace_to_nid_split(
        pa_trace, 
        seq![trace.last()], 
        pa, 
        pa_level - 1,
    ...
    assert(Self::valid_nid(pa)) by { Self::lemma_trace_to_nid_from_root(trace.drop_last()); };
