...
impl<M: PageMeta> Page<M> {
    ...
    pub fn from_unused(paddr: Paddr, metadata: MetaSlotInner, Ghost(owner):Ghost<PageOwner>, Tracked(specific_perm):Tracked<SpecificPagePerm>,Tracked(s): Tracked<AbstractState>)
            ...
            res.1@.invariants(),
    {
        ...
        if !cas.is_ok(){
            ...
            proof{ s_panic.insert_pageperm(paddr, perm); }
    ...
    #[rustc_allow_incoherent_impl]
    pub fn into_raw(self, Ghost(owner): Ghost<PageOwner>, Tracked(s): Tracked<AbstractState>)
    ...
    #[rustc_allow_incoherent_impl]
    pub fn from_raw(paddr:Paddr,Ghost(owner):Ghost<PageOwner>,Tracked(s):Tracked<AbstractState>)
    ...
    #[rustc_allow_incoherent_impl]
    pub fn clone(&self, Ghost(owner): Ghost<PageOwner>, Tracked(s): Tracked<AbstractState>)
        -> (res: (Self, Tracked<AbstractState>))
        requires
            self.has_valid_paddr(),
            self.relate_model(s.get_page(self.paddr())),
    ...
    #[rustc_allow_incoherent_impl]
    pub fn reference_count<'a>(&'a self, p_slot:Tracked<&'a simple_pptr::PointsTo<MetaSlot>>, p_ref_count: Tracked<&PermissionU32>) -> (res: u32)
    requires
        self.inv_ptr(),
    ...
    #[rustc_allow_incoherent_impl]
    pub fn ref_count<'a>(&'a self, p_slot:Tracked<&'a simple_pptr::PointsTo<MetaSlot>>) -> (res: &'a PAtomicU32)
    requires
        self.inv_ptr(),
        p_slot@.pptr() == self.ptr,
        p_slot@.is_init(),
    ...
    #[rustc_allow_incoherent_impl]
    fn try_from(dyn_page: DynPage, Tracked(s): Tracked<AbstractState>) -> (res: (Result<Self,DynPage>, Tracked<AbstractState>))
    requires
        dyn_page.inv_ptr(),
    ...
    #[rustc_allow_incoherent_impl]
    pub fn into(self, Tracked(s):Tracked<AbstractState>) -> (res: (DynPage, Tracked<AbstractState>))
    requires
        self.has_valid_paddr(),
        self.relate_model(s.get_page(self.paddr())),
        s.get_page(self.paddr()).state != PageState::Unused,
...
pub fn inc_page_ref_count(paddr: Paddr, Tracked(s): Tracked<AbstractState>)
...
#[rustc_allow_incoherent_impl]
pub fn into_frame(self, Tracked(s):Tracked<AbstractState>) -> (res: (Frame, Tracked<AbstractState>))
requires
    self.has_valid_paddr(),
    self.relate_model(s.get_page(self.paddr())),
    s.get_page(self.paddr()).state == PageState::Untyped,
    s.get_page(self.paddr()).usage == PageUsage::Frame,
    exists |thread_id:int| s.get_page(self.paddr()).owners.contains(PageOwner::User{thread_id}),
    s.invariants(),
ensures
    res.0.inv(),
    res.0.relate_model(res.1@.get_page(self.paddr())),
...
#[rustc_allow_incoherent_impl]
pub fn try_into<M:PageMeta>(self, Tracked(s): Tracked<AbstractState>) -> (res: (Result<Page<M>,DynPage>, Tracked<AbstractState>))
requires
    self.inv_ptr(),
    self.relate_model(s.get_page(self.paddr())),
    s.invariants(),
