...
impl PageModel {
    ...
    pub proof fn insert_keep_owners_invariants(&self, owner: PageOwner)
        ...
        ensures
            forall|owner_| #[trigger]
                self.owners.insert(owner).contains(owner_) ==> owner_.as_usage() == self.usage,
    {
        let new_owners = self.owners.insert(owner);
        assert forall|owner_| #[trigger] new_owners.contains(owner_) implies owner_.as_usage()
            == self.usage by {
            if owner_ == owner {
    ...
    pub proof fn remove_keep_owners_invariants(&self, owner: PageOwner)
        ...
        ensures
            forall|owner_| #[trigger]
                self.owners.remove(owner).contains(owner_) ==> owner_.as_usage() == self.usage,
    {
    ...
    #[rustc_allow_incoherent_impl]
    pub proof fn transfer_spec_satisfy_invariants(
        &self,
        ...
        requires
            self.invariants(),
        ensures
            self.transfer_spec(prev_owner, new_owner).is_some() ==> self.transfer_spec(
                prev_owner,
                new_owner,
            ).unwrap().invariants(),
    {
        if self.transfer_spec(prev_owner, new_owner).is_some() {
            ...
            assert forall|owner| #[trigger] model.owners.contains(owner) implies owner.as_usage()
                == model.usage by {
                if owner == new_owner {
            ...
            ).unwrap().invariants(),
    {
        if self.share_with_spec(owner).is_some() {
            ...
            assert forall|owner_| #[trigger] model.owners.contains(owner_) implies owner_.as_usage()
                == model.usage by {
                if owner_ == owner {
    ...
    pub proof fn leak_owner_spec_satisfy_invariants(&self, owner: PageOwner)
        requires
            self.invariants(),
        ensures
            self.leak_owner_spec(owner).is_some() ==> self.leak_owner_spec(
                owner,
            ).unwrap().invariants(),
    {
    ...
    pub proof fn inc_spec_satisfy_invariants(&self)
        requires
            self.invariants(),
        ensures
            self.inc_spec().is_some() ==> self.inc_spec().unwrap().invariants(),
    {
    }
