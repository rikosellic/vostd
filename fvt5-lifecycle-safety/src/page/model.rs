...
impl PageModel {
            ...
            self.share_with_spec(owner) == Some(res),
    {
        ...
        PageModel { ref_count: self.ref_count + 1, owners: self.owners.insert(owner), ..*self }
    }
            ...
            self.leak_owner_spec(owner) == Some(res),
    {
        ...
        PageModel { owners: self.owners.remove(owner), ..*self }
    }
    ...
    {
        self.insert_keep_owners_invariants(owner);
        PageModel { owners: self.owners.insert(owner), ..*self }
    }
    ...
    {
        PageModel { ref_count: self.ref_count + 1, ..*self }
    }
            ...
            self.transfer_spec(prev_owner, new_owner) == Some(res),
    {
        ...
        PageModel { owners: self.owners.remove(prev_owner).insert(new_owner), ..*self }
    }
}
