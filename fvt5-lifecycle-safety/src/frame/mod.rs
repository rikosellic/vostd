...
impl Frame{
    ...
    pub fn from(page: Page<FrameMeta>, Tracked(s):Tracked<AbstractState>) -> (res: (Self, Tracked<AbstractState>))
    requires
        ...
        exists |thread_id:int| s.get_page(page.paddr()).owners.contains(PageOwner::User{thread_id}),
        s.invariants(),
    ensures
        res.0.inv(),
    ...
    #[rustc_allow_incoherent_impl]
    pub fn try_from(page:DynPage, Tracked(s):Tracked<AbstractState>) -> (res: (Result<Frame,DynPage>, Tracked<AbstractState>))
    requires
        page.inv_ptr(),
        page.relate_model(s.get_page(page.paddr())),
        s.invariants(),
        s.get_page(page.paddr()).state != PageState::Unused,
        s.get_page(page.paddr()).usage == PageUsage::Frame ==> s.get_page(page.paddr()).state == PageState::Untyped,
        s.get_page(page.paddr()).state == PageState::Untyped ==> exists |thread_id:int| s.get_page(page.paddr()).owners.contains(PageOwner::User{thread_id}),
    ensures
        ...
        res.1@ == s,
    {
        ...
        match res {
            Ok(p) => {let (p,Tracked(s)) = p.into_frame(Tracked(s)); (Ok(p),Tracked(s))},
