...
impl <'a, M: PageTableMode> Cursor<'a, M> {
            ...
            self.barrier_va.start < self.barrier_va.end,
    {
        ...
        let locked_path =
            s.page_table.get_path(model.locked_subtree);
            ...
            model.inv(s),
    {
        ...
        forall |i: int| 0 <= i < nodes.len() ==>
            {
                i < barrier_lv ==> #[trigger] self.guards[i].is_none()
            } && {
                barrier_lv <= i <= lv ==>
                    #[trigger]
                    self.guards[i].is_some() &&
                    self.guards[i].unwrap().relate(nodes[i]) &&
                    nodes[i].is_locked
            }
    }
    ...
    &&& forall |i: int| 0 < i <= NR_LEVELS ==>
        {i == NR_LEVELS - self.guard_level ==>
            self.guards[i].is_some()} &&
        {i != NR_LEVELS - self.guard_level ==>
            self.guards[i].is_none()}
    }
    ...
    #[rustc_allow_incoherent_impl]
    pub fn virt_addr(&self, Tracked(s): Tracked<AbstractState>, Tracked(model): Tracked<ConcreteCursor>) -> (res: Vaddr)
        requires
            self.inv(),
            self.relate(s, model),
    {
    ...
    #[rustc_allow_incoherent_impl]
    pub open spec fn cur_entry_spec(&self, s: AbstractState, model: &ConcreteCursor) -> (res: Entry<'a>) {
    ...
    #[rustc_allow_incoherent_impl]
    #[verifier::external_body]
    fn cur_entry(&self, Tracked(s): Tracked<AbstractState>, Tracked(model): Tracked<&ConcreteCursor>) -> (res: Entry<'a>)
        requires
            self.inv(),
        ensures
            ...
            res@.invariants()
    {
        ...
        let ghost barrier_end_path = PageTableTreePathModel::from_va((self.barrier_va.end - 1) as usize);
        ...
        assert(
            forall |i:int|
                    !(i == NR_LEVELS - self.guard_level) ==>
                    self.guards.view().index(i) is None) by { admit() };
    ...
    #[rustc_allow_incoherent_impl]
    #[verifier::external_body]
    pub fn jump(&mut self, va: Vaddr, Tracked(s): Tracked<AbstractState>, Tracked(model): Tracked<&ConcreteCursor>)
    ...
    #[rustc_allow_incoherent_impl]
    pub proof fn perms_at_path(s: Ghost<AbstractState>,
        ...
        res.1@.invariants()
    {
        ...
        if va.start % PagingConsts::BASE_PAGE_SIZE() != 0 || va.end % PagingConsts::BASE_PAGE_SIZE() != 0 {
            ...
            s.get_page(cur_pt_addr).usage == PageUsage::PageTable,
        {
            ...
            let pt_arr_ptr:ArrayPtr<PageTableEntry,CONST_NR_ENTRIES> = ArrayPtr::from_addr(paddr_to_vaddr(cur_pt_addr));
            ...
            if cur_pte.is_present() {
                if cur_pte.is_last(cursor.level) {
                    ...
                    proof {
                        cur_path = Ghost(PageTableTreePathModel::from_path(cur_path@.inner.push_tail(start_idx)));
        ...
        let (raw, s1) = RawPageTableNode::from_raw_parts_unleaked(cur_pt_addr, cursor.level, Tracked(s));
