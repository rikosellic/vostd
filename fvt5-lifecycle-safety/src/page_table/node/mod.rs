...
#[rustc_allow_incoherent_impl]
pub fn inc_ref_count(&self, Tracked(s): Tracked<AbstractState>)
...
#[rustc_allow_incoherent_impl]
pub fn clone_shallow(&self, Tracked(s):Tracked<AbstractState>) -> (res: (Self,Tracked<AbstractState>))
    requires
        self.has_valid_paddr(),
        self.relate_model(s.get_page(self.paddr())),
        s.invariants(),
        ...
        res.1@.invariants(),
{
    ...
    //Ghost operations
    let tracked end_state = increased_state.restore_owner_at(self.paddr(),RAW_PAGE_TABLE_NODE_OWNER);
    proof{
        let ghost page_model = increased_state.get_page(self.paddr());
        let ghost end_page_model = end_state.get_page(self.paddr());
        s.lemma_update_page_model_spec_same_paddr_twice(self.paddr(), page_model, end_page_model);
    }
...
#[rustc_allow_incoherent_impl]
pub fn into(self, Tracked(s):Tracked<AbstractState>) -> (res: (Page<PageTablePageMeta>, Tracked<AbstractState>))
requires
    self.has_valid_paddr(),
    self.relate_model(s.get_page(self.paddr())),
    s.invariants(),
...
#[rustc_allow_incoherent_impl]
pub fn from_raw_parts(paddr: Paddr, level: PagingLevel, Tracked(s): Tracked<AbstractState>) -> (res: (Self, Tracked<AbstractState>))
    requires
...
#[rustc_allow_incoherent_impl]
pub fn from_raw_parts_unleaked(paddr: Paddr, level: PagingLevel, Tracked(s): Tracked<AbstractState>) -> (res: (Self, Tracked<AbstractState>))
    requires
        paddr % PAGE_SIZE_SPEC() == 0,
        paddr < MAX_PADDR_SPEC(),
        s.invariants(),
...
#[rustc_allow_incoherent_impl]
pub fn first_activate(&self, Tracked(s):Tracked<AbstractState>) -> (res: Tracked<AbstractState>)
    requires
        self.has_valid_paddr(),
        self.relate_model(s.get_page(self.paddr())),
        s.invariants(),
        ...
        res@.invariants(),
{
    ...
    //Ghost operations
    proof{
        s.lemma_inc_then_restore_owner_implies_share_with(self.paddr(),PAGE_TABLE_CPU_OWNER,increased_state);
    }
...
#[rustc_allow_incoherent_impl]
pub fn lock(self, Tracked(s):Tracked<AbstractState>) -> (res: (PageTableNode, Tracked<AbstractState>))
    requires
        self.has_valid_paddr(),
        self.relate_model(s.get_page(self.paddr())),
        s.invariants(),
        ...
        res.1@.invariants(),
{
    ...
    while page.
        meta_pt(Tracked(&perm.ptr_perm),Tracked(perm.inner_perm.tracked_borrow())).
        lock.
        compare_exchange(Tracked(&mut pagetable_model.lock),0, 1).
        is_err()
    invariant
        SpecificPagePerm::PT(pagetable_model).match_meta_slot_inner(perm.get_inner_perm().value()),
    {
        spin_loop();
    }
    ...
    //Ghost operations
    let tracked end_state = s.transfer_at(page.paddr(),RAW_PAGE_TABLE_NODE_OWNER,PAGE_TABLE_NODE_OWNER);
    (PageTableNode {
        page,
    }, Tracked(end_state))
}
...
#[rustc_allow_incoherent_impl]
pub fn from(raw: RawPageTableNode, Tracked(s):Tracked<AbstractState>) -> (res: (Self, Tracked<AbstractState>))
requires
    raw.has_valid_paddr(),
    ...
    res.1@ == s,
{
    ...
    proof{
        s.lemma_leak_owner_then_restore_owner_implies_unchanged(paddr,RAW_PAGE_TABLE_NODE_OWNER, forget_state);
    }

    //unsafe { Page::<PageTablePageMeta<E, C>>::from_raw(raw.paddr()) }
    Page::<PageTablePageMeta>::from_raw(paddr,Ghost(RAW_PAGE_TABLE_NODE_OWNER),Tracked(forget_state))
}
...
#[rustc_allow_incoherent_impl]
pub fn clone_raw(&self, Tracked(s):Tracked<AbstractState>) -> (res: (RawPageTableNode,Tracked<AbstractState>))
    requires
        self.inv(),
        s.invariants(),
        s.get_page(self.paddr()).ref_count < u32::MAX,
...

{
    ...
    let (cloned_page, Tracked(cloned_state)) = self.page.clone(Ghost(RAW_PAGE_TABLE_NODE_OWNER),Tracked(s));
    ...
    proof{
        ...
        cloned_state.lemma_leak_owner_then_restore_owner_implies_unchanged(paddr,RAW_PAGE_TABLE_NODE_OWNER, forget_state);
    ...
    let level = self.level(Tracked(&perm.ptr_perm),Tracked(inner_perm), Tracked(pagetable_perm));
    RawPageTableNode::from_raw_parts(paddr, level, Tracked(forget_state))
}

#[rustc_allow_incoherent_impl]
pub fn into_raw(self, Tracked(s):Tracked<AbstractState>) -> (res: (RawPageTableNode, Tracked<AbstractState>))
    requires
        self.inv(),
        s.invariants(),
        self.relate_model(s.get_page(self.page.paddr())),
        ...
        res.1@.invariants(),
{
    ...
    self.page.meta_pt(Tracked(&mut perm.ptr_perm),Tracked(perm.inner_perm.tracked_borrow())).lock.store(Tracked(&mut pagetable_model.lock),0);

    let level = self.level(Tracked(&perm.ptr_perm),Tracked(perm.inner_perm.tracked_borrow()), Tracked(&pagetable_model.inner));
    proof{
        perm.specific_perm = SpecificPagePerm::PT(pagetable_model);
    ...
    proof{
        s.lemma_leak_owner_then_restore_owner_implies_transfer(paddr,PAGE_TABLE_NODE_OWNER,RAW_PAGE_TABLE_NODE_OWNER, forget_state);
    }
