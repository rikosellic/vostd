...
impl PageTableEntryTrait for PageTableEntry {
    ...
    fn new_page(paddr: Paddr, level: PagingLevel, prop: PageProperty) -> Self {
        ...
        proof {
            ...
            assert((addr | hp | flags) & present_mask != 0) by (bit_vector)
                requires
                    flags & present_mask != 0,
            ;
            assert(Self(addr | hp | flags).is_last(level));
            assert((addr | hp | flags) & 0xF_FFFF_FFFF_F000 == addr) by (bit_vector)
                requires
        ...
        proof {
            ...
            assert((0b1usize | 0b10 | 0b100) & 0b1 != 0) by (bit_vector)
                requires
                    flags & 0xF_FFFF_FFFF_F000 == 0,
            ;
            assert((paddr & 0xF_FFFF_FFFF_F000 | flags) & 0xF_FFFF_FFFF_F000 == paddr
                & 0xF_FFFF_FFFF_F000) by (bit_vector)
                requires
                    flags & 0xF_FFFF_FFFF_F000 == 0,
            ;
    ...
    open spec fn prop_spec(&self) -> PageProperty {
        let flags: u8 = if self.0 & PageTableFlags::PRESENT().bits() == 0 {
            0
        } else {
            PageFlags::R().bits()
        ...
        let priv_flags: u8 = if self.0 & PageTableFlags::USER().bits() == 0 {
            0
        } else {
    ...
    fn prop(&self) -> PageProperty {
        ...
        let priv_flags = parse_flags!(self.0, PageTableFlags::USER(), PrivilegedPageFlags::USER())
...
impl PageTableEntry {
    ...
    pub open spec fn format_flags_spec(prop: PageProperty) -> usize {
        ...
        PageTableFlags::PRESENT().bits() | if flags & PageFlags::R().bits() == 0 {
            0
        } else {
            PageTableFlags::PRESENT().bits()
        } | if flags & PageFlags::W().bits() == 0 {
            0
        } else {
            PageTableFlags::WRITABLE().bits()
        } | if flags & PageFlags::ACCESSED().bits() == 0 {
            0
        } else {
            PageTableFlags::ACCESSED().bits()
        } | if flags & PageFlags::DIRTY().bits() == 0 {
            0
        } else {
            PageTableFlags::DIRTY().bits()
        } | if !flags & PageFlags::X().bits() == 0 {
            0
        } else {
            PageTableFlags::NO_EXECUTE().bits()
        } | if priv_flags & PrivilegedPageFlags::USER().bits() == 0 {
            0
        } else {
            PageTableFlags::USER().bits()
        } | if priv_flags & PrivilegedPageFlags::GLOBAL().bits() == 0 {
            0
        } else {
            ...
            Self::format_flags(prop) & Self::PHYS_ADDR_MASK() == 0,
    {
        let flags: u8 = prop.flags.bits();
        let priv_flags: u8 = prop.priv_flags.bits();
        let flag_1 = if flags & 0b1 == 0 {
            0
        } else {
            0b1
        };
        let flag_2 = if flags & 0b10 == 0 {
            0
        } else {
            0b10
        };
        let flag_3 = if flags & 0b1000 == 0 {
            0
        } else {
            0b100000
        };
        let flag_4 = if flags & 0b10000 == 0 {
            0
        } else {
            0b1000000
        };
        let flag_5 = if !flags & 0b100 == 0 {
            0
        } else {
            1usize << 63
        };
        let flag_6 = if priv_flags & 0b1 == 0 {
            0
        } else {
            0b100
        };
        let flag_7 = if priv_flags & 0b10 == 0 {
            0
        } else {
            0b1_00000000
        };
        ...
        let present_mask = PageTableFlags::PRESENT().bits();
        assert((present_mask | flag_1 | flag_2 | flag_3 | flag_4 | flag_5 | flag_6 | flag_7
            | flag_8) & present_mask != 0) by (bit_vector)
            requires
                present_mask == 0b1,
        ;
        assert((present_mask | flag_1 | flag_2 | flag_3 | flag_4 | flag_5 | flag_6 | flag_7
            | flag_8) & 0xF_FFFF_FFFF_F000 == 0) by (bit_vector)
            requires
                present_mask == 0b1,
                flag_1 == 0 || flag_1 == 0b1,
    ...
    pub fn format_huge_page(level: PagingLevel) -> u64
        returns
            Self::format_huge_page_spec(level),
    {
        if level == 1 {
            0
        } else {
...
pub proof fn lemma_parse_flags_page_table_flags(flag: usize)
    ensures
        parse_flags!(flag, PageTableFlags::WRITABLE(), PageFlags::W()) == if flag
            & PageTableFlags::WRITABLE().bits() == 0 {
            0
        } else {
            PageFlags::W().bits()
        } <= u8::MAX,
        parse_flags!(flag, PageTableFlags::PRESENT(), PageFlags::R()) == if flag
            & PageTableFlags::PRESENT().bits() == 0 {
            0
        } else {
            PageFlags::R().bits()
        } <= u8::MAX,
        parse_flags!(flag, PageTableFlags::ACCESSED(), PageFlags::ACCESSED()) == if flag
            & PageTableFlags::ACCESSED().bits() == 0 {
            0
        } else {
            PageFlags::ACCESSED().bits()
        } <= u8::MAX,
        parse_flags!(!flag, PageTableFlags::NO_EXECUTE(), PageFlags::X()) == if !flag
            & PageTableFlags::NO_EXECUTE().bits() == 0 {
            0
        } else {
            PageFlags::X().bits()
        } <= u8::MAX,
        parse_flags!(flag, PageTableFlags::DIRTY(), PageFlags::DIRTY()) == if flag
            & PageTableFlags::DIRTY().bits() == 0 {
            0
        } else {
            PageFlags::DIRTY().bits()
        } <= u8::MAX,
        parse_flags!(flag, PageTableFlags::USER(), PrivilegedPageFlags::USER()) == if flag
            & PageTableFlags::USER().bits() == 0 {
            0
        } else {
            PrivilegedPageFlags::USER().bits()
        } <= u8::MAX,
        parse_flags!(flag, PageTableFlags::GLOBAL(), PrivilegedPageFlags::GLOBAL()) == if flag
...
pub proof fn lemma_parse_flags_page_flags(flag: u8)
    ensures
        parse_flags!(flag, PageFlags::W(), PageTableFlags::WRITABLE()) == if flag
            & PageFlags::W().bits() == 0 {
            0
        } else {
            PageTableFlags::WRITABLE().bits()
        } <= usize::MAX,
        parse_flags!(flag, PageFlags::R(), PageTableFlags::PRESENT()) == if flag
            & PageFlags::R().bits() == 0 {
            0
        } else {
            PageTableFlags::PRESENT().bits()
        } <= usize::MAX,
        parse_flags!(flag, PageFlags::ACCESSED(), PageTableFlags::ACCESSED()) == if flag
            & PageFlags::ACCESSED().bits() == 0 {
            0
        } else {
            PageTableFlags::ACCESSED().bits()
        } <= usize::MAX,
        parse_flags!(!flag, PageFlags::X(), PageTableFlags::NO_EXECUTE()) == if !flag
            & PageFlags::X().bits() == 0 {
            0
        } else {
            PageTableFlags::NO_EXECUTE().bits()
        } <= usize::MAX,
        parse_flags!(flag, PageFlags::DIRTY(), PageTableFlags::DIRTY()) == if flag
            & PageFlags::DIRTY().bits() == 0 {
            0
        } else {
            PageTableFlags::DIRTY().bits()
        } <= usize::MAX,
...
pub proof fn lemma_parse_flags_previleged_page_flags(flag: u8)
    ensures
        parse_flags!(flag, PrivilegedPageFlags::USER(), PageTableFlags::USER()) == if flag
            & PrivilegedPageFlags::USER().bits() == 0 {
            0
        } else {
            PageTableFlags::USER().bits()
        } <= usize::MAX,
        parse_flags!(flag, PrivilegedPageFlags::GLOBAL(), PageTableFlags::GLOBAL()) == if flag
            & PrivilegedPageFlags::GLOBAL().bits() == 0 {
            0
        } else {
            PageTableFlags::GLOBAL().bits()
        } <= usize::MAX,
