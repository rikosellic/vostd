            ...
            ==> handle_count(s.frames, frame_to_index(s.frames[fid].paddr)) == 1,
{
    ...
    s.regions.contains_valid_frame_paddr(paddr);
        ...
        final(s).inv(),
{
    ...
    assert(pre_rc_target == handle_count(old_frames, target_idx) + pre_paths_target
        + pre_cover_target);
    assert(old_regions.slot_owners[target_idx].metadata_perm.not_empty()
        ...
        final(s).inv(),
{
    ...
    assert forall|u: UniqueId| #[trigger] s.unique_frames.dom().contains(u) implies {
        ...
        s.regions.contains_valid_frame_paddr(s.unique_frames[u].paddr);
        ...
        final(s).inv(),
{
    ...
    s.regions.contains_valid_frame_paddr(p);
        ...
        match res {
            Option::Some(entry) => {
                ...
                assert forall|u: UniqueId| #[trigger] s.unique_frames.dom().contains(u) implies {
                    ...
                    assert(old_regions.slot_owners[u_idx].ref_count() == REF_COUNT_UNIQUE);
                    assert(old_regions.slot_owners[u_idx].ref_count() != REF_COUNT_UNUSED);
        ...
        final(s).inv(),
{
    ...
    assert forall|u: UniqueId| #[trigger] s.unique_frames.dom().contains(u) implies {
        ...
        s.regions.contains_valid_frame_paddr(u_paddr);
        // Old UNIQUE validity at `u`.
        assert(old_regions.slot_owners[u_idx].ref_count() == REF_COUNT_UNIQUE);
        assert(old_regions.slot_owners[u_idx].usage is Frame);
        ...
        final(s).inv(),
{
    ...
    assert forall|sid_other: SegmentId, paddr_c: Paddr|
        ...
        // `cov_0 <= idx < max_meta_slots()` via `contains_valid_frame_paddr`
        ...
        s.regions.contains_valid_frame_paddr(paddr_c);
    ...
    assert forall|fid_other: FrameId| #[trigger]
        ...
        s.regions.contains_valid_frame_paddr(s.frames[fid_other].paddr);
