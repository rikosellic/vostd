...
impl<M: AnyFrameMeta + Repr<MetaSlot>> CursorMut<M> {
            ...
            self.wf(owner.move_next_owner_spec()),
    {
        ...
        self.current = match self.current {
            ...
            Some(current) => current.borrow(
                Tracked(owner.list_own.perms.tracked_borrow(owner.index)),
            ).next,
            None => self.list.borrow(Tracked(&owner.list_perm)).front,
        };
            ...
            self.wf(owner.move_prev_owner_spec()),
    {
        ...
        self.current = match self.current {
            ...
            None => self.list.borrow(Tracked(&owner.list_perm)).back,
        };
    ...
    */
            ...
            res.is_some() ==> res.unwrap().1@.frame_link_inv(),
    {
        ...
        let opt_prev = frame_meta.borrow(Tracked(&frame_own.meta_perm)).prev;
            ...
            owner.inv(),
    {
        ...
        if let Some(current) = self.current {
            ...
            let opt_prev = current.borrow(Tracked(&cur_perm)).prev;
            proof {
            ...
            if let Some(back) = self.list.borrow(Tracked(&mut owner.list_perm)).back {
