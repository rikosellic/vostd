...
impl<M: AnyFrameMeta> CursorModel<M> {
    ...
    pub open spec fn remove(self) -> Self {
        ...
        let fore = if self.fore.len() > 0 {
            LinkedListModel::update_next(self.fore, self.fore.len() - 1, cur.next)
        } else {
            self.fore
        };
        let rear = if self.rear.len() > 1 {
            LinkedListModel::update_prev(self.rear, 1, cur.prev)
        } else {
            self.rear
        };
    ...
    pub open spec fn insert(self, link: LinkModel<M>) -> Self {
        ...
        let rear = if self.rear.len() > 0 {
            LinkedListModel::update_prev(self.rear, 0, link.prev)
        } else {
            self.rear
        };
        let fore = if fore.len() > 0 {
            LinkedListModel::update_next(self.fore, self.fore.len() - 1, link.next)
        } else {
            fore
        };

        Self { fore: fore, rear: rear, list_model: self.list_model }
    }
}
...
impl<M: AnyFrameMeta> LinkedListOwner<M> {
    ...
    pub open spec fn remove_list_spec(list: Seq<LinkOwner<M>>, i: int) -> Seq<LinkOwner<M>> {
        let cur = list[i];
        let list = if i > 0 {
            LinkedListOwner::update_next(list, i - 1, cur.next)
        } else {
            list
        };
        let list = if i < list.len() - 1 {
            LinkedListOwner::update_prev(list, i + 1, cur.prev)
        } else {
            list
        };
        list.remove(i)
...
impl<M: AnyFrameMeta> CursorOwner<M> {
    ...
    pub open spec fn insert_owner_spec(self, owner: LinkOwner<M>) -> Self {
        Self {
            list_own: LinkedListOwner {
                list: self.list_own.list.insert(self.index, owner),
                ..self.list_own
            },
