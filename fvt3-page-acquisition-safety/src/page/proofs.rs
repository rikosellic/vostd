...
pub proof fn relate_meta_slot_implies_relate_model(&self, slot: &MetaSlot,
        model: &MetaSlotModel)
        requires
...
pub proof fn new_spec_satisfy_invariants(&self, usage: PageUsage, owner: PageOwner)
    requires
        self.invariants()
    ensures
        self.new_spec(usage, owner).is_some() ==>
        self.new_spec(usage, owner).unwrap().invariants()
{}

pub proof fn transfer_spec_satisfy_invariants(&self, prev_owner: PageOwner,
    new_owner: PageOwner)
    requires
        self.invariants()
    ensures
        ...
        self.transfer_spec(prev_owner, new_owner).unwrap().invariants()
{
    if self.state == PageState::Unused {
        ...
        assert(self.owners.contains(prev_owner) == false);
        assert(self.transfer_spec(prev_owner, new_owner).is_some() == false);
    } else {
