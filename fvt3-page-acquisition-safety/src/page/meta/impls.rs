...
{
    match M::get_usage() {
        PageUsage::Frame => Self {
            _frame: manually_drop(super::FrameMeta::default()),
        },
...
pub fn init() -> (res: (Self, Tracked<MetaSlotModel>))
    ensures
        ...
        res.1@.ref_count == 0_u32,
        res.1@.state == MetaSlotState::Unused,
{
    let (cell, Tracked(cell_perm)) = PCell::empty();
    let usage = AtomicU8::new(
        Ghost(cell),
        0,
        Tracked(ActualUsage::Unused(cell_perm)));
...
{
    let tracked mut unwrap_model: MetaSlotModel = model.get();
    let tracked mut perm: PointsTo<MetaSlotInner> = unwrap_model.inner_perm.tracked_unwrap().get();
