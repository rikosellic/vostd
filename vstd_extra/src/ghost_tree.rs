...
impl <const N: usize> TreePath<N> {
    ...
    pub broadcast proof fn drop_head_property(self)
        ...
        ensures
            ...
            forall |i: int| 0 <= i < self.0.drop_first().len() ==>
                #[trigger]
                self.0.drop_first().index(i) == self.index(i + 1),
    { }
    ...
    pub broadcast proof fn drop_last_property(self)
        ...
        ensures
            ...
            forall |i: int| 0 <= i < self.0.drop_last().len() ==>
                #[trigger]
                self.0.drop_last().index(i) == self.index(i),
    { }
    ...
    pub broadcast proof fn pop_tail_property(self)
        ...
        ensures
        #[trigger]
            self.pop_tail().0 == self.index(self.len() as int - 1),
            self.pop_tail().1.len() == self.len() - 1,
            Self::elem_inv(self.pop_tail().0),
            self.pop_tail().1.inv(),
    {
    ...
    pub proof fn push_head_property_index(self, hd: usize)
        ...
        ensures
            ...
            forall |i: int| 0 <= i < self.len() ==>
                self.push_head(hd).index(i + 1) == self.index(i),
    { }
            ...
            self.push_head(hd).inv(),
    {
        ...
        assert forall |i: int| 0 <= i < s1.len() implies
            Self::elem_inv( #[trigger] s1.index(i)) by {
            if i == 0 {
    ...
    pub proof fn push_tail_property_index(self, val: usize)
        ...
        ensures
            ...
            forall |i: int| 0 <= i < self.len() ==>  #[trigger] 
                self.push_tail(val).index(i) == self.index(i),
    { }
...
impl <T: TreeNodeValue, const N: usize, const L: usize> Node<T, N, L> {
    ...
    #[verifier::inline]
    pub open spec fn size() -> nat { N as nat}

    #[verifier::inline]
    pub open spec fn max_depth() -> nat { L as nat }
    ...
    pub broadcast proof fn insert_property(self, key: usize, node: Self)
        ...
        #[trigger]
            ...
            forall |i: int| 0 <= i < Self::size() && i != key ==>
                #[trigger]
                self.insert(key, node).children[i] == self.children[i],
    { }
    ...
    pub broadcast proof fn remove_property(self, key: usize)
        ...
        #[trigger]
            ...
            forall |i: int| 0 <= i < Self::size() && i != key ==>
                #[trigger]
                self.remove(key).children[i] == self.children[i],
    { }
    ...
    pub broadcast proof fn child_property(self, key: usize)
        ...
        ensures
        #[trigger]
            self.child(key) == self.children[key as int],
    { }
    ...
    pub broadcast proof fn child_property_cases(self, key: usize)
        ...
        ensures
            self.level == L - 1 ==>
                #[trigger]
                self.child(key).is_none(),
            self.level < L - 1 && self.children[key as int].is_none() ==>
                #[trigger]
                self.child(key).is_none(),
            self.level < L - 1 && self.children[key as int].is_some() ==>
                #[trigger]
    ...
    pub broadcast proof fn lemma_insert_child_is_child(self, key: usize, node: Self)
        ...
        ensures
            #[trigger]
            self.insert(key, node).child(key) == Some(node),
    {
        self.insert_property(key, node);
        self.child_property(key);
    }
    ...
    pub broadcast proof fn lemma_remove_child_is_none(self, key: usize)
        ...
        ensures
            #[trigger]
            self.remove(key).child(key).is_none(),
    {
    ...
    pub open spec fn recursive_insert(self, path: TreePath<N>, node: Self) 
        -> Self
        recommends
            self.inv(),
            path.inv(),
    ...
    pub broadcast proof fn lemma_recursive_insert_path_empty_identical(self, 
        path: TreePath<N>, node: Self)
        requires
            self.inv(),
            node.inv(),
            path.inv(),
    ...
    pub broadcast proof fn lemma_recursive_insert_path_len_1(self,
        path: TreePath<N>, node: Self)
        requires
            self.inv(),
            node.inv(),
        ...
        ensures
            #[trigger]
            self.recursive_insert(path, node) == self.insert(path.index(0), node),
    { }

    pub broadcast proof fn lemma_recursive_insert_path_len_step(self,
        path: TreePath<N>, node: Self)
        requires
            self.inv(),
            path.inv(),
            ...
            path.len() > 1,
        ensures
            self.child(path.index(0)).is_some() ==>
                #[trigger]
                self.recursive_insert(path, node) ==
                self.insert(
                    path.index(0),
                    self.child(path.index(0)).unwrap().recursive_insert(
                        path.pop_head().1, node)
                ),
            self.child(path.index(0)).is_none() ==>
                #[trigger]
                self.recursive_insert(path, node) ==
                self.insert(
                    path.index(0),
                    Node::new(self.level + 1).recursive_insert(
                        path.pop_head().1, node)
                ),
    { }

    pub broadcast proof fn lemma_recursive_insert_preserves_inv(self,
        path: TreePath<N>, node: Self)
        requires
            self.inv(),
            path.inv(),
    ...
    pub proof fn recursive_trace_up_to(self, path1: TreePath<N>, path2: TreePath<N>, n: int)
        ...
        ensures
            forall |i:int|
                0 <= i <= n ==>
                self.recursive_trace(path1)[i] == self.recursive_trace(path2)[i]
        decreases
            n,
    { admit() }
    ...
    pub broadcast proof fn lemma_recursive_visited_node_inv(self, path: TreePath<N>)
        requires
            ...
            path.len() < L - self.level,
        ensures
            forall |i: int| 0 <= i < self.recursive_visit(path).len() ==>
                #[trigger] self.recursive_visit(path).index(i).inv(),
        decreases 
            path.len()
    {
        if path.is_empty() {
    ...
    pub broadcast proof fn lemma_recursive_visited_node_levels(self, path: TreePath<N>)
        ...
        ensures
            forall |i: int| 0 <= i < self.recursive_visit(path).len() ==>
                #[trigger] 
                self.recursive_visit(path).index(i).level == self.level + i + 1,
        decreases
            path.len()
    {
        if path.is_empty() {
            ...
            self.recursive_visit(path).index(0) == self.child(path.index(0)).unwrap(),
    {
        if path.len() == 0 {
            ...
            if self.child(path.index(0)).is_none() {
                ...
                assert(self.recursive_visit(path).len() > 0);
                assert(self.recursive_visit(path) =~= seq![self.child(path.index(0)).unwrap()]
                    .add(self.child(path.index(0)).unwrap().recursive_visit(path.pop_head().1)));
                assert(self.recursive_visit(path).index(0) == self.child(path.index(0)).unwrap());
            }
        }
    }
    ...
    pub broadcast proof fn lemma_recursive_visit_induction(self, path: TreePath<N>)
        ...
        ensures
            #[trigger]
            self.recursive_visit(path) == seq![self.child(path.pop_head().0).unwrap()]
                .add(self.child(path.pop_head().0).unwrap().recursive_visit(path.pop_head().1)),
    {
        ...
        if path.len() == 1 {
            ...
            if c.is_none() {
                ...
                assert(tl.len() > 0);
                assert(tl.len() == path.len() - 1);
                assert(self.recursive_visit(path) =~= seq![c]
                    .add(c.recursive_visit(tl)));
            }
        }
    }
    ...
    pub broadcast proof fn lemma_recursive_visit_one_is_child(self, path: TreePath<N>)
        ...
        ensures
            self.child(path.index(0)).is_some() ==>
                #[trigger]
                self.recursive_visit(path) =~= seq![self.child(path.index(0)).unwrap()],
            self.child(path.index(0)).is_none() ==>
                #[trigger]
                self.recursive_visit(path) =~= seq![],
    {
        if self.child(path.index(0)).is_none() {
            assert(self.recursive_visit(path).len() == 0);
        } else {
    ...
    pub broadcast proof fn lemma_child_on_subtree(self, key: usize)
        ...
        ensures
            #[trigger]
            self.on_subtree(self.child(key).unwrap()),
    {
    ...
    pub broadcast proof fn lemma_insert_on_subtree(self, key: usize, node: Self)
        ...
        ensures
            #[trigger]
            self.insert(key, node).on_subtree(node),
    {
    ...
    pub broadcast proof fn lemma_recursive_insert_on_subtree(self, path: TreePath<N>, node: Self)
        ...
        ensures
            #[trigger]
            self.recursive_insert(path, node).on_subtree(node),
        decreases
            path.len()
    {
        admit(); // TODO
    ...
    pub broadcast proof fn lemma_recursive_remove_not_on_subtree(self, path: TreePath<N>, node: Self)
        requires
            self.inv(),
            path.inv(),
            node.inv(),
    ...
    pub broadcast proof fn lemma_recursive_visit_on_subtree(self, path: TreePath<N>)
        ...
        ensures
            forall |i: int| 0 <= i < self.recursive_visit(path).len() ==>
                 #[trigger]
                self.on_subtree(self.recursive_visit(path)[i]),
    {
        admit(); // TODO
    }
    ...
    pub broadcast proof fn remaining_level_decreases(self)
        ...
        ensures
            forall |i: int| 0 <= i < Self::size() &&
                #[trigger]
                self.children[i].is_some() ==>
                    (L - self.children[i].unwrap().level) < (L - self.level),
    {
...
pub open spec fn path_between<T: TreeNodeValue, const N: usize, const L: usize>(
    src: Node<T, N, L>, dst: Node<T, N, L>) -> TreePath<N>
    recommends
        src.inv(),
        dst.inv(),
        src.level <= dst.level,
...
#[verifier::external_body]
pub broadcast proof fn path_between_properties
    <T: TreeNodeValue, const N: usize, const L: usize>
    (src: Node<T, N, L>, dst: Node<T, N, L>)
    requires
        src.inv(),
        dst.inv(),
        src.level <= dst.level,
    ...
    ensures
        ...
        dst.level == src.level ==>
            path_between(src, dst).is_empty() &&
            src == dst,
        dst.level > src.level ==>
            src.recursive_visit(path_between(src, dst)).last() == dst,
{ }
...
impl <T: TreeNodeValue, const N: usize, const L: usize> Tree<T, N, L> {
    ...
    pub broadcast proof fn visit_is_recursive_visit(self, path: TreePath<N>)
        ...
        ensures
            #[trigger]
            self.visit(path) == self.root.recursive_visit(path),
    { }
    ...
    ensures
        forall |i:int|
            0 <= i <= n ==>
            self.trace(path1)[i] == self.trace(path2)[i]
    decreases
        n,
    { admit() }
    ...
    pub proof fn seek_trace_next(self, path:TreePath<N>, idx: usize)
        ensures
            self.seek(path).unwrap().children[idx as int].unwrap().value ==
    ...
    pub broadcast proof fn visited_nodes_inv(self, path: TreePath<N>)
        ...
        ensures
            ...
            forall |i: int| 0 <= i < self.visit(path).len() ==>
                #[trigger] self.visit(path).index(i).inv(),
    {
    ...
    pub broadcast proof fn get_path_properties(self, node: Node<T, N, L>)
        ...
        ensures
            ...
            node.level == 0 ==>
                #[trigger] self.get_path(node).is_empty(),
            node.level > 0 ==>
                #[trigger] self.visit(self.get_path(node)).last() == node,
    {
